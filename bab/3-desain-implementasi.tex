...
\begin{figure}[H]
  ...
  \includegraphics[scale=0.95]{gambar/bab3-dataset-citra.png}

  \caption{Dataset kosakata saya}
  \label{fig:datasetMethod}
\end{figure}

Adapaun bahasa isyarat yang akan digunakan pada tugas akhir ini meliputi 12 kosakata isyarat BISINDO yang memiliki konteks kalimat tanya yang umum digunakan sehari – hari. Nantinya untuk setiap kosakata tersebut akan disimpan sebanyak 30 jumlah frame. Kosakata yang akan digunakan adalah sebagai berikut:
...
Adapun digunakan tambahan 4 buah isyarat tambahan di luar isyarat BISINDO untuk mempermudah kontrol sistem penerjemah. Isyarat tersebut meliputi isyarat mulai (\textit{start}), isyarat \textit{standby}, isyarat hapus kata (\textit{delete}), dan isyarat terjemah (\textit{translate}).  
\begin{longtable}{|c|c|c|}
...
\begin{figure}[H]
  ...
  \includegraphics[scale=0.4]{gambar/bab3-estimasi-pose.png}
...
\begin{figure}[H]
  ...
  \includegraphics[scale=0.55]{gambar/bab3-model-lstm.png}

  \caption{Model LSTM}
  \label{fig:modelLSTM}
\end{figure}

Dalam melakukan klasifikasi terhadap pose atau gerakan isyarat yang dilakukan, digunakan model LSTM sekuensial. Model ini tersusun atas serangkaian layer yang akan diproses secara bertahap dari awal hingga akhir. Layer pertama merupakan layer \textit{TimeDistributed} yang di dalamnya terdapat layer \textit{Dense} dengan jumlah unit sebesar 265, fungsi aktivasi '\textit{tanh}' dan input dengan bentuk (30, 108). Penggunaan \textit{TimeDistributed} memastikan bahwa untuk setiap frame akan diproses secara independen, dimana akan diimplementasikan layer \textit{Dense} yang konsisten pada setiap frame yang ada. Perlu diperhatikan bahwa bentuk (30, 108) diartikan sebagai bahwa akan terdapat 30 jumlah frame, dimana untuk setiap frame akan diekstrak total 108 fitur. Setelahnya dilanjutkan dengan layer LSTM yang memiliki jumlah unit 128, return\_sequences=False, fungsi aktivasi '\textit{tanh}', dan input dalam bentuk (30, 108). Setelah layer LSTM diberikan layer Dropout dengan nilai 0.5 untuk menghindari adanya kelebihan weight yang mana dapat mengakibatkan \textit{overfitting} pada model. Dilanjutkan dengan layer \textit{Dense} dengan jumlah unit 128 dan fungsi aktivasi 'relu'. Setelah layer \textit{Dense} juga diberikan layer Dropout dengan nilai 0.5 untuk menghindari adanya kelebihan weight yang mana dapat mengakibatkan \textit{overfitting} pada model. Layer terakhir adalah layer \textit{Dense} yang menggunakan fungsi aktivasi '\textit{softmax}'. Jumlah unit di layer ini sesuai dengan actions.shape[0], yang berarti jumlah unit sama dengan jumlah aksi atau kategori yang mungkin. Fungsi aktivasi '\textit{softmax}' memastikan bahwa keluaran dari model ini adalah distribusi probabilitas di atas semua kategori yang mungkin, dengan total semua probabilitas sama dengan 1. Model ini kemudian dikompilasi menggunakan optimizer Adam, yang dikenal karena performa dan efisiensinya dalam pelatihan jaringan saraf, dan menggunakan \textit{categorical crossentropy} sebagai fungsi \textit{loss} karena ini adalah masalah klasifikasi multi-kelas, dengan metrik akurasi kategorikal untuk memantau performa model selama proses \textit{training}

\subsection{Sistem Kontrol}
Adapun dalam pembuatan tugas akhir ini, sistem kontrol adalah serangkaian proses yang akan mengatur bagaimana nantinya bahasa isyarat akan dideteksi, melakukan penanggulangan terhadap adanya kesalahan pendeteksian, menyatukan berbagai kata dalam bentuk kalimat, penghapusan kosakata, dan penerjemahan kalimat dalam bentuk suara. Sistem kontrol akan dibagi menjadi 2, yaitu program deteksi bahasa isyarat dan program pembentukan kalimat.
